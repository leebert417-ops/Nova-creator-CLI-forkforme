...
- **📂 角色卡解包**：将现有的角色卡 JSON 还原为源文件，方便修改和维护
...
如果你有现成的角色卡 JSON 文件，想要修改内容，可以使用解包工具将其还原为源文件：

### 解包角色卡

```bash
node unpack-card.js <角色卡JSON路径> [输出目录]
```

示例：
```bash
node unpack-card.js ./夜雨旧梦.json ./作品
```

解包后会生成：
- **完整的源文件目录**：包含所有世界书条目、开场白、状态栏等
- **打包配置文件**：可直接用于重新打包的 YAML 配置
...
2. 使用配置文件重新打包：
   ```bash
   node build-card.js 作品/角色卡名称/角色卡名称.yaml
   ```

这种工作流程特别适合：
- **维护老作品**：将旧的角色卡转换为源文件管理
- **批量修改**：使用编辑器的查找替换功能批量修改内容
- **版本控制**：将源文件纳入 Git 管理，方便追踪修改历史
- **协作开发**：多人可以分别修改不同的文件
